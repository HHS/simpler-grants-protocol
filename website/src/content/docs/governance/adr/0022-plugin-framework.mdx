...
6. **Plugin authors provide `toCommon` / `fromCommon` as functions; mappings are one way to generate them.** The SDK exposes `buildTransforms()` / `build_transforms()` as a public utility that wraps the existing mapping runtimes (`website/src/lib/transformation.ts`, `common_grants_sdk.utils.transformation`). `PluginExtensions.schemas.<Object>` gains an optional `mappings` key carrying JSON-safe `toCommon` / `fromCommon` mapping objects; when those are declared and no explicit transform is supplied in `schemas.<Object>`, `definePlugin()` invokes `buildTransforms()` automatically. Mappings for each direction are author-provided — `buildTransforms()` does not invert one direction into the other, because many-to-one handlers like `switch` are not reversible. First-release handler set is `field` and `switch`; the reverse direction is limited to the reversible subset until [ADR-0017](/governance/adr/0017-mapping-format) defines reversible mapping semantics. Implementation tracked under [#744](https://github.com/HHS/simpler-grants-protocol/issues/744); handler expansion under [#745](https://github.com/HHS/simpler-grants-protocol/issues/745).

7. **Validate plugin output at runtime; validate mapping definitions at utility-invocation time.** `definePlugin()` wraps the final `toCommon` / `fromCommon` (however produced) with `schema.parse` / `model_validate`, because compile-time types do not guarantee runtime shape. Mappings passed to `buildTransforms()` are checked at the call site, failing fast on structural errors, unknown handlers, or unresolvable field paths. A non-throwing `safeTransform` / `safe_transform` variant returns aggregated per-item errors for batch transformation. Tracked under [#744](https://github.com/HHS/simpler-grants-protocol/issues/744).

8. **Custom handlers are registered per utility call, not globally.** `buildTransforms()` accepts an optional `handlers` argument for registering additional handler names. Per-call scoping keeps behavior explicit and testable; name collisions with the default set (`field`, `switch`) raise at `buildTransforms()` call time rather than silently shadowing them. Handler-name lookup uses own-property access (`Object.hasOwn` in TypeScript, explicit dict `in` checks in Python) to avoid resolving inherited prototype or class-dict members (`__proto__`, `constructor`) when mapping JSON is reconstituted from untrusted sources via `mergeExtensions()`. Tracked under [#745](https://github.com/HHS/simpler-grants-protocol/issues/745).

9. **Transformation errors carry structured context.** SDK-emitted transformation errors extend a single `PluginError` base with `path`, `handler`, `sourceValue`, and `cause` fields, enabling programmatic reasoning without parsing error text. `sourceValue` may contain PII when transforming applicant data; adopters are responsible for redacting it before logging or re-raising, and the SDK does not redact by default. Specific subclasses are documented alongside the implementation in [#744](https://github.com/HHS/simpler-grants-protocol/issues/744) / [#745](https://github.com/HHS/simpler-grants-protocol/issues/745).
...
// Outputs are validated at runtime with schema.parse / model_validate regardless of
// how they were produced (see Decision #7).
...
// Handler signature matches ADR-0017 runtime conventions; full specification in #745.
...
// time; unknown handlers, structural errors, and unresolvable field paths raise
// PluginError subclasses. The optional `handlers` argument registers custom handler
// names for this call only — collisions with the default set (`field`, `switch`)
// raise at load time (see Decision #8).
...
// Non-throwing batch variant. Applies a transform to each input and returns aggregated
// per-item results and errors — useful when partial failure is preferable to aborting
// the whole batch (see Decision #7). Exact return-type shape — including whether
// errors carry the failing input's index for position attribution — is finalized in #744.
function safeTransform<TIn, TOut>(
  transform: (input: TIn) => TOut,
  inputs: TIn[],
): { results: (TOut | null)[]; errors: PluginError[] };

// Base class for SDK-emitted transformation errors (see Decision #9). Specific
// subclasses (e.g. UnknownHandlerError, PathResolutionError, ValidationFailedError)
// are documented alongside the implementation in #744 / #745.
interface PluginError extends Error {
  path?: string; // field path where the failure occurred
  handler?: string; // handler name that raised, if applicable
  sourceValue?: unknown; // value that couldn't be transformed
  cause?: unknown; // underlying error, when wrapped
}
```
...
# Outputs are validated at runtime with model_validate regardless of how they were
# produced (see Decision #7).
def define_plugin(
    meta: PluginMeta | None = None,
    get_client: Callable[[ClientConfig], Client] | None = None,
    extensions: PluginExtensions | None = None,
...
# PluginError subclasses. The optional `handlers` argument registers custom handler
# names for this call only — collisions with the default set ('field', 'switch')
# raise at build_transforms() call time (see Decision #8). Concrete typing of the
# returned callables is provided by the caller (ObjectSchemas[TNative, TCommon]);
# the utility itself is generic over Any to avoid unbound TypeVar drift.
...
def safe_transform(
    ...
    inputs: list[Any],
) -> tuple[list[Any | None], list['PluginError']]: ...

# Base class for SDK-emitted transformation errors (see Decision #9). Specific
# subclasses (e.g. UnknownHandlerError, PathResolutionError, ValidationFailedError)
# are documented alongside the implementation in #744 / #745. The explicit __init__
# (rather than bare class-level annotations) keeps the four attributes as per-instance
# state instead of class variables shared across instances.
class PluginError(Exception):
    def __init__(
        self,
        message: str = '',
        *,
        path: str | None = None,              # field path where the failure occurred
        handler: str | None = None,           # handler name that raised, if applicable
        source_value: Any = None,             # value that couldn't be transformed
        cause: BaseException | None = None,   # underlying error, when wrapped
    ) -> None:
...
- **Positive consequences**
  ...
  - Structured `PluginError` hierarchy (`path`, `handler`, `sourceValue`, `cause`) lets adopters reason about transformation failures programmatically without parsing error text; a non-throwing `safeTransform` variant supports batch processing where partial failure is preferable to abort
