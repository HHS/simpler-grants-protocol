...
8. **Custom handlers are registered per utility call, not globally.** `buildTransforms()` accepts an optional `handlers` argument for registering additional handler names. Per-call scoping keeps behavior explicit and testable; name collisions with the default set (`field`, `switch`) raise at load time rather than silently shadowing them. Tracked under [#745](https://github.com/HHS/simpler-grants-protocol/issues/745).

9. **Transformation errors carry structured context.** SDK-emitted transformation errors extend a single `PluginError` base with `path`, `handler`, `sourceValue`, and `cause` fields, enabling programmatic reasoning without parsing error text. Specific subclasses are documented alongside the implementation in [#744](https://github.com/HHS/simpler-grants-protocol/issues/744) / [#745](https://github.com/HHS/simpler-grants-protocol/issues/745).
...
// the whole batch (see Decision #7).
...
- **Positive consequences**
  ...
  - Mappings travel with serializable `extensions`, so adopters can ship declarative transforms alongside custom fields without hand-written functions — the SDK compiles them at plugin-construction time
  - Output validation happens at `definePlugin()` wrap-time and mapping validation happens at `buildTransforms()` call-time, catching shape errors at the nearest reasonable boundary rather than at first use
  ...
  - Custom handlers are registered per-call on `buildTransforms()`, not globally — behavior stays explicit and testable, and collisions with the default set raise at load time
