...
- **Positive consequences**
  ...
  - Structured `PluginError` hierarchy lets adopters reason about transformation failures programmatically without parsing error text; a non-throwing `safeTransform` variant supports batch processing where partial failure is preferable to abort
