...
5. **Multiple IDs per registry:** `id` for the primary, plus an optional `allIds` array of `{ id, status }` objects for additional and archived IDs.
...
Identifier:
  ...
  properties:
    ...
    allIds:
      type: array
      description: Additional IDs in this registry beyond the primary `id` (e.g. fiscal sponsor EINs, retired UEIs)
      items:
        type: object
        properties:
          id:
...
```json
{
  "identifiers": {
    "ein": {
      "registry": "US-EIN",
      "id": "12-3456789",
      "allIds": [{ "id": "98-7654321", "status": "active" }]
    },
    "uei": {
      "registry": "US-UEI",
      "id": "AB0123456789"
...
ein.id; // "12-3456789"
ein.allIds; // [{ id: "98-7654321", status: "active" }]
```
...
In both options below, the primary ID stays at the top level as `id`. The difference is how additional IDs (fiscal sponsor EINs, retired UEIs, etc.) are represented.

#### Side-by-side comparison

| Criteria                                         | Array of `{ id, status }` (rec.) | Flat array of strings |
| ------------------------------------------------ | :------------------------------: | :-------------------: |
| Distinguishes active vs. archived additional IDs |                ✅                |          ❌           |
| Unambiguous primary value                        |                ✅                |          ✅           |
| Zero overhead for the common single-value case   |                ✅                |          ✅           |
| Extensible without breaking changes              |                ✅                |          🟡           |
| Simple to populate and read                      |                🟡                |          ✅           |
| Works with native `contains()` / `in` membership |                ❌                |          ✅           |
...
- **Pros**
  - `id` is the single clear primary; `allIds` only appears when there are alternates.
  ...
  - Consumers that don't care about alternates can ignore `allIds` entirely.
...
- **Cons**
  ...
  - Doesn't support native `contains()` or `in` membership checks (e.g. `"98-7654321" in ein.allIds`). Consumers have to project first: `ein.allIds.map(v => v.id).includes("98-7654321")`.
...
- **Pros**
  ...
  - Primary ID is implicit from `id`.
  - Works with native `contains()` / `in` membership checks (e.g. `"98-7654321" in ein.allIds`).
- **Cons**
  - Can't tell which additional IDs are still valid and which are archived.
