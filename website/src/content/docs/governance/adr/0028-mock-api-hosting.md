...
The deciding fact is a hosting decision made outside this ADR: on 2026-08-19 the team chose to migrate the website from GitHub Pages to Cloudflare (the switch [ADR 0003](/governance/adr/0003-website-hosting/) itself anticipated if server-side functions ever became a requirement). GitHub Pages runs no code, so while it remained the production host, the integrated shape was structurally impossible there and the experiments' original recommendation was the standalone Worker (3a). That migration has since completed — name servers moved, every merge to `main` verified against a parallel deploy at beta.commongrants.org, and the DNS cutover retiring the GitHub Pages deploy ([#1145](https://github.com/HHS/simpler-grants-protocol/pull/1145)) — so 3b's one decisive cost is gone, and same-origin serving makes it the simpler shape. The details of that reversal are under [Option 3](#option-3-real-mock-endpoint-on-cloudflare-chosen-as-3b) below.
...
- **No launch gate:** the mock is served unconditionally, in PR previews and on production alike. A `MOCK_API_ENABLED` build flag originally kept Swagger UI's Execute buttons off in production, because the still-live GitHub Pages deploy could not serve the endpoint; once the cutover made it a switch that was never off, [#1149](https://github.com/HHS/simpler-grants-protocol/pull/1149) removed it. Execute is still offered only on the verbs the mock implements — the docs panel reads its `supportedSubmitMethods` from the router's own `SUPPORTED_METHODS`.
...
- The host framework sits in front of the mock's handlers. Astro's `checkOrigin` CSRF guard answers a plain-text 403 to any non-GET request carrying neither a matching `Origin` nor a non-form `Content-Type`, where a standalone Worker returned a protocol-shaped body. This reaches routes the mock does serve, not just unimplemented verbs: `PUT /applications/{appId}/submit` takes no request body, so a bare `curl -X PUT` against it sends no `Content-Type` and is refused before the handler runs. Sending any non-form content type (`-H 'Content-Type: application/json'`) clears the guard, and the docs panel's own same-origin requests never trip it. It stays on deliberately — disabling a site-wide CSRF guard to improve one error body's shape would be the worse trade.
...
- The mock's availability is tied to the site's: they build, deploy, and roll back as one unit, so the endpoint cannot ship — or be rolled back — independently of the docs. The one-time form of this cost is already paid: shipping was sequenced behind the hosting migration, and the DNS cutover that retired the GitHub Pages deploy is what let the mock go live on production.
...
- **The reversal:** on 2026-08-19 the team decided to migrate the website to Cloudflare regardless — the migration was already planned work (parallel deploy on every merge, verification on beta.commongrants.org, DNS cutover, then retiring the Pages deploy), not a cost of choosing 3b. That triggered the experiment findings' own carve-out ("if the team independently decides to migrate the docs site to Cloudflare, 3b becomes the better shape"), and the 3b branch became the launch vehicle.
...
- **Cons**
  - Requires the docs host to run code — viable only because of the independently-decided Cloudflare migration, which has since landed: the cutover retired the GitHub Pages deploy, and the mock now serves on production with no gate.
