...
4. **Delete the legacy monorepo examples**: `examples/ca-opportunity-example/`, `examples/pa-opportunity-example/`, the root `examples/README.md`, and the unreferenced `examples/opportunity_example.py`. The active [`agilesix/cg-api-ca`](https://github.com/agilesix/cg-api-ca) and [`agilesix/cg-api-pa`](https://github.com/agilesix/cg-api-pa) repositories are separate, current, and unaffected; the monorepo copies are 2025-era FastAPI projects pinned five minor lines behind the published SDK, linked from no website page, and not synchronized with those repositories. Comparison with the active CA and PA repositories found no legacy code, fixture, or documentation that requires migration. The active adapters model the complete source records and archive raw inputs, while their transformations supersede the legacy mappings. The legacy 2099 date sentinel and incompatible UUID namespaces will not be retained.
5. **Provide a 90-day passive bridge for already-published CLIs.** The window begins when the retirement release publishes. During it, the existing manifest and the template files it references remain as a frozen, explicitly unsupported snapshot: no dependency updates, CI, or compatibility guarantee against future package releases. Implementation work must name an owner accountable for removing the snapshot at the deadline. The root CA/PA examples are not fetched by `cg init` and are outside the bridge.
...
- **Pros**
  ...
  - No ongoing owner, cadence, or freshness automation is needed for artifacts that no longer exist; the temporary bridge carries only a deadline and a removal owner.
- **Cons**
...
## Compatibility for already-published CLIs
...
The decision is a 90-day passive bridge beginning when the retirement release publishes. The existing manifest and the template files it references stay in place as a frozen, explicitly unsupported snapshot — no dependency updates, no CI, no compatibility guarantee against future package releases — and are removed at the deadline. The root CA/PA examples are not fetched by `cg init` and are outside the bridge.

Immediate removal was the closest alternative. Evidence of active use is weak and the CLI is pre-1.0, so deleting the paths when the implementation lands would be defensible. It was not selected because usage cannot be measured directly, and a bounded passive bridge provides cautious deprecation without creating a permanent maintenance commitment.

The bridge fixes its start at the retirement release and its deadline 90 days later. Implementation work must name an owner accountable for the removal at that deadline.

Implementation work must also settle two details that the bridge decision does not determine:
...
| Deprecating a published CLI surface               | No precedent in ADRs 0001–0027                                              | New territory — addressed in the compatibility section above, not a divergence                                                                                                                                               |
...
| Reliability                                       | Error paths and partial-failure modes                                       | Addressed — the 90-day bridge bounds the compatibility period; implementation work must deliberately choose the old-CLI failure behavior after removal                                                                       |
...
| Operations                                        | Deploy, rollback, migration, versioning                                     | Addressed — release and removal sequencing in the compatibility section; documentation migration in Option 3's requirements                                                                                                  |
