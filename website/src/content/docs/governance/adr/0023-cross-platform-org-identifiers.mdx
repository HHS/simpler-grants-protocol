---
title: Cross-platform organization identifiers
...
The protocol will replace the hardcoded `ein`, `uei`, and `duns` fields on `OrganizationBase` with a generic `identifiers` collection. The same `IdentifierCollection` field will be reused across other core models, with each model extending it with its own well-known keys.
...
2. **Collection structure:** A map keyed by short well-known aliases, with a catch-all `otherIds` map for extensions.
...
8. **Registry governance:** CommonGrants maintains a registry catalog that wraps org-id.guide where applicable and adds CommonGrants-specific metadata (schema URLs, kind, lookup templates). A documented promotion flow moves a registry from `otherIds` to a well-known key on the relevant model.
...
### Well-known keys and extensions

Each `IdentifierCollection` has two tiers of keys:

- **Well-known (root-level) keys** are defined by the protocol on a per-model basis. They are the short camelCase aliases that every CommonGrants-compliant system is expected to recognize for that model. On `OrganizationBase`, these include `ein`, `uei`, and `duns`. On `OpportunityBase`, they include `opportunityNumber` and `cfda`. Well-known keys have a registered entry in the CommonGrants registry catalog and are validated against a per-key JSON schema.
- **`otherIds`** is a catch-all map for identifiers that don't yet have a well-known key on that model. Adopters can put any identifier here using any camelCase alias they choose (e.g. `candidBridge`, `statePortalId`). The value must still follow the `Identifier` schema, but the key itself is not reserved or validated by the protocol.

**When to use each:**

- If the registry has a well-known key on the model, publish it under that key.
...
**Promotion flow: `otherIds` to well-known key**

An identifier type moves from `otherIds` to a first-class well-known key through a documented flow. This ensures that new keys are vetted, validated, and released on the standard protocol cadence instead of being added ad hoc.
...
2. **Review.** Reviewers evaluate: is the registry broadly applicable? Is it authoritative (a named issuer)? Can values be validated?
3. **Catalog entry.** On acceptance, a PR adds a catalog file (`commongrants.org/registries/<code>.yaml`) with the alias, scope, kind, schema, and lookup template.
4. **Model extension.** For each relevant model, the alias is added as a well-known key on that model's `IdentifierCollection` extension (e.g. `ein`, `uei` on `OrgIdentifierCollection`).
5. **Release.** Changes ship in the next minor protocol version with the appropriate `@Versioning.added`.
6. **Adopter migration.** Adopters can continue to publish the identifier under `otherIds` during a deprecation window, then move it to the well-known key.

See [Evaluation - Registry governance](#evaluation---registry-governance) for the full rationale behind this flow.

### Questions

- **[Top-level ID format](#evaluation---top-level-id-format):** Should the top-level `id` be a plain UUID, a prefixed UUID, or a registry-scoped composite string?
- **[Collection structure](#evaluation---collection-structure):** Should the identifier collection be a flat array or a keyed map?
- **[Map key format](#evaluation---map-key-format):** If the collection is a map, should the keys be raw registry codes or short model-defined aliases?
- **[Identifier metadata structure](#evaluation---identifier-metadata-structure):** How much registry metadata belongs on each identifier instance, and how much should live in an external catalog?
- **[Multiple IDs per registry](#evaluation---multiple-ids-per-registry):** How should the protocol represent multiple IDs that share the same registry, including archived versions?
- **[Parent organization representation](#evaluation---parent-organization-representation):** What should a parent organization reference carry?
- **[Registry code format](#evaluation---registry-code-format):** Should registry codes follow org-id.guide, a uniform namespaced format, or both?
- **[Registry governance](#evaluation---registry-governance):** Who maintains the registry catalog, and how does a registry get promoted from `otherIds` to a well-known key?
...
<details>
<summary>IdentifierCollection (base)</summary>
...
## Evaluation - Top-level ID format
...
## Evaluation - Collection structure
...
## Evaluation - Identifier metadata structure
...
## Evaluation - Multiple IDs per registry
...
## Evaluation - Parent organization representation
...
## Evaluation - Registry code format
...
## Evaluation - Registry governance

_Who maintains the registry catalog, and how does a registry get promoted from `otherIds` to a well-known key?_
...
| Clear `otherIds` → well-known key flow               |      ✅       |        🟡         |       ✅        |
...
Any identifier that starts in `otherIds` can become a first-class well-known key through the promotion flow documented in [Well-known keys and extensions](#well-known-keys-and-extensions).
