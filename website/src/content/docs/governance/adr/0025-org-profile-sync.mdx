...
Required scope: `org:write`. The body is a JSON Merge Patch (RFC 7396): include a field to set it, leave it out to keep it unchanged, or send `null` to clear it. The receiver figures out who made the change and which system it came from based on the token and request context, rather than fields in the request body (see [provenance](#provenance)).
...
Required scope: `org.changes:write`. Creates a change from a JSON Merge Patch body. A receiver that applies it right away returns `accepted`; one that routes it through review returns `pending` with a `Location` header for the new change (see [acceptance semantics](#acceptance)). A direct `PATCH /orgs/{orgId}` is the shorthand for the apply-now case, and both land in `GET /orgs/{orgId}/changes`. The exact request envelope is left to the follow-up spec.
...
5. Verify `scope` covers the operation: a direct `PATCH` needs `org:write` (or `org:sync`), a `POST /changes` submission needs `org.changes:write`, and a read needs `org:read`.
...
:::note[Bottom line]
Supporting both flows is the right call if we want to cover both unattended service sync and access a human has consented to, and we're okay implementing and documenting two flows instead of one.
:::
...
Here the token stands in for a one-time human consent that can't be recreated in code, so the refresh token becomes the durable record of that consent and lets a platform keep syncing without re-prompting. Senders MUST keep refresh tokens server-side and MUST NOT let them reach a browser or other client-side environment.
...
:::note[Bottom line]
Client credentials only works if we expect sync to be purely service-to-service, but it can't represent a human consenting to act on a specific org's behalf, which rules out the platform-on-behalf-of-org case.
:::
...
:::note[Bottom line]
PKCE only works if every sync starts with a human consenting, but it forces an awkward consent step onto purely automated service sync.
:::
...
:::note[Bottom line]
A JWT is the right call if we want a receiver to validate and authorize a request without calling back to the issuer every time, and we're okay accepting that revocation isn't immediate and keeping token lifetimes short.
:::

Access tokens are JWTs ([RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519)). A receiver checks the signature against the issuer's JWKS and reads `scope` and `org_id` straight from the token (the payload shown under [Required JWT claims](#required-jwt-claims)) to make its decision, with no extra round trip. Short lifetimes keep the revocation window small: 15 to 60 minutes for access tokens, refresh tokens up to a year and rotated on each use, and authorization codes good for 60 seconds and a single use.
...
:::note[Bottom line]
Opaque tokens are the right call if immediate revocation is a hard requirement, but they put a network call to the authorization server in the path of every sync request.
:::
...
- **Cons**
  ...
  - Less turn-key against the JWT-centric SDK ecosystem adopters already use.
...
:::note[Bottom line]
One token per org is the right call if we want the standard, widely understood model where a token is bound to a single organization, and we're okay with a client that syncs several orgs using a separate token for each.
:::
...
This mirrors how the big providers scope a token to a tenant: [Auth0's Organizations](https://auth0.com/docs/manage-users/organizations) feature issues a token with a single `org_id` (plus `org_name`), and [Microsoft Entra](https://learn.microsoft.com/en-us/entra/identity-platform/id-token-claims-reference) names a single tenant in `tid`. [GitHub](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens/managing-your-personal-access-tokens) arrives at the same shape from the other direction, where a fine-grained token is limited to a single organization and the org is a property of the token rather than a scope string. We namespace the claim following the [collision-safe convention Auth0 requires for custom claims](https://auth0.com/docs/secure/tokens/json-web-tokens/create-custom-claims).

A token can also omit `org_id` entirely to act on every organization the subject can access, with the receiver resolving which those are from its own policy. This is how a client lists or reads across orgs without enumerating org IDs in the token, the same shape [Google](https://developers.google.com/identity/protocols/oauth2/scopes) uses, where the token names operations and the resource server resolves the accessible resources. A no-`org_id` token with `org:write` can update any accessible org, which a receiver may choose to restrict.
...
- **Pros**
  ...
  - Per-org access levels need no extra machinery; they're just separate tokens.
  - Nothing to reconcile, since one token names exactly one org.
- **Cons**
  - A client that syncs many orgs manages a separate token for each.

#### Option 2: One multi-org token with uniform access

:::note[Bottom line]
A single token with an `org_ids` list works if we want one token to cover many orgs at once, but the same scopes then apply to every org in the list, and it's a custom claim with no standard behind it.
:::
...
:::note[Bottom line]
RFC 9396 `authorization_details` is the right call if we need one token to express different access levels for different orgs, but it's heavier for issuers and receivers and isn't universally supported.
:::
...
```json
{
  "authorization_details": [
    { "type": "org_profile", "identifier": "01912a8b-7c3d-7890-abcd-ef1234567890", "actions": ["read"] },
    { "type": "org_profile", "identifier": "02a33c9d-1e2f-4a5b-8c7d-9e0f1a2b3c4d", "actions": ["read", "write"] }
  ]
}
```
...
:::note[Bottom line]
Supporting both is the right call if we want a simple, familiar `PATCH` for trusted direct edits and a safer `POST /changes` for reviewable proposals, both landing in one auditable ledger, and we're okay building two write entry points instead of one. Adopters expose either or both.
:::
...
Both take the same JSON Merge Patch body and both show up in `GET /orgs/{orgId}/changes`, so history is uniform no matter how a write arrived. The two paths use different scopes (`org:write` for `PATCH`, `org.changes:write` for `POST /changes`, mirroring `org.changes:read`), so a deployment can grant a partner the ability to propose changes without granting direct-write access. A deployment that only makes trusted in-system edits can expose just `PATCH`; one that only accepts changesets from outside can expose just `POST /changes`; a system doing both exposes both. This mirrors the two OAuth flows: one contract, and adopters use the entry points that fit.

The `POST /changes` shape is well-trodden. The most familiar version is a payment or an order: you `POST` to create it and get back an object with an `id` and a `status` you check later, exactly how [Stripe](https://docs.stripe.com/api/refunds/object) models a refund or a payment intent. [Gerrit](https://gerrit-review.googlesource.com/Documentation/rest-api-changes.html) applies the same idea to profile-style edits with a literal `/changes/` collection whose entries each have a status, GitHub does it with pull requests, and the broader async convention returns [`202 Accepted`](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.3.3) with a status monitor to poll (as in [Google's long-running operations](https://google.aip.dev/151) and the [Microsoft REST guidelines](https://github.com/microsoft/api-guidelines/blob/vNext/graph/patterns/long-running-operations.md)).

- **Pros**
  - A trusted writer gets a terse, familiar `PATCH`; an external proposer gets a reviewable submission, without forcing either into the other's shape.
...
:::note[Bottom line]
`PATCH` alone is the right call if every writer is trusted to apply edits directly, but it gives an external system no way to propose a change for review, only to apply one.
:::
...
:::note[Bottom line]
Routing every write through `POST /changes` is the right call if we want one uniform, reviewable write path, but it puts create-then-check ceremony on even the simplest trusted edit.
:::
...
:::note[Bottom line]
`PUT` only works if every writer always has the complete, canonical record. A strict replace clears fields the sender doesn't model, which is the cross-system hazard this contract exists to avoid, so it isn't offered as a write path.
:::
...
:::note[Bottom line]
Merge Patch is the right call if we want the patch body to look like the org record adopters already work with, and we're okay giving up element-level array edits and letting `null` mean "clear."
:::
...
:::note[Bottom line]
JSON Patch is the right call if we need element-level array edits and an explicit list of operations, but it adds a second format keyed off JSON Pointer paths that doesn't match the org record shape.
:::
...
```json
[
  { "op": "replace", "path": "/mission", "value": "To expand access to community health resources." },
...
:::note[Bottom line]
Field masks are the right call if we want an explicit list of the fields being changed alongside the values, but they add a parallel mask field and are less common in the REST/JSON world.
:::
...
:::note[Bottom line]
A version list is the right call if we want direct retrieval of a record at a known version and a history that tolerates squashing, and we're okay with more storage than a change log and diffing snapshots for per-field detail.
:::
...
```json
{ "datasetVersion": 9, "modifiedAt": "2026-06-20T14:30:00Z", "patch": { "mission": "..." }, "snapshot": { "name": "...", "mission": "..." } }
```

Fetching a record at a known version is a direct lookup with no event replay, diffing two versions is "fetch both and compare," and dropping intermediate versions (the squash the contract allows) still leaves a navigable timeline. This composes with the dataset-version number reads and writes already return. An optional `?at={timestamp}` parameter on the read is a complementary pattern: it lets a consumer ask for "the record as of then" without learning a separate history shape. The version-list shape is common in the wild, like [Google Drive revisions](https://developers.google.com/drive/api/reference/rest/v3/revisions), Confluence versions, and MediaWiki history.
...
:::note[Bottom line]
A change log is the right call if the priority is cheap storage and fine-grained activity tracking, but it pushes state reconstruction onto consumers and doesn't squash well.
:::

History is a stream of field-level deltas, sized by activity rather than record size:

```json
{ "field": "mission", "from": "To provide...", "to": "To expand...", "at": "2026-06-20T14:30:00Z" }
```
...
:::note[Bottom line]
A point-in-time query alone is the right call if we want the smallest surface and cheap access to historical state, but consumers can't browse history without already knowing a version or timestamp, and the server pays the most.
:::
...
:::note[Bottom line]
Letting a change's `status` represent either an immediate result or a queued review is the right call if we want one contract for both immediate-apply and approval-gated deployments, and we're okay with a receiver that always applies immediately still returning a `status`.
:::
...
- **Cons**
  - A receiver that always applies immediately still returns a `status` it may not need, though it can steer trusted clients to `PATCH`.
...
:::note[Bottom line]
Resolving every change on submission works if no deployment ever needs review, but then `POST /changes` can never say "received, not yet applied" and adds little over `PATCH`.
:::
...
:::note[Bottom line]
Queuing every change works if all changes are reviewed, but it forces a receiver that would apply immediately to model a `pending` state it never uses.
:::
...
:::note[Bottom line]
Deriving provenance from the validated token is the right call if we want a trail a sender can't forge, and we're okay resolving who made a change from the token's own identity rather than a self-declared body field.
:::
...
:::note[Bottom line]
Taking provenance from the request body is simplest for a sender that wants to attribute a change to an arbitrary actor, but the receiver has no way to trust it: anyone who can send a write can set it to anything.
:::
