...
A funding opportunity, such as a grant or loan, is the core data model for the Funding API.
...
<Tabs>
  ...
  | fundingDetails | FundingDetails        | Yes      | Details about the funding available                                          |
  | keyDates       | OppTimeline           | Yes      | Key dates for the opportunity, such as when the application opens and closes |
  ...
  | createdAt      | isoDatetime           | Yes      | The timestamp (in UTC) at which the record was created                       |
  | lastModifiedAt | isoDatetime           | Yes      | The timestamp (in UTC) at which the record was last modified                 |
  </TabItem>
