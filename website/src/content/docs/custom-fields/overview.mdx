...
[Custom fields](/protocol/fields/custom-field/) solve this problem by providing a structured way to extend CommonGrants models with additional data. Each custom field includes:
...
// Define a value schema for complex types
...
// opportunity.customFields?.agency?.value.code  → string | undefined
// opportunity.customFields?.category?.value     → string | undefined
```
...
# Define specs for each custom field
fields = {
...
# Create a new model class with typed custom fields
Opportunity = OpportunityBase.with_custom_fields(
    custom_fields=fields, model_name="Opportunity"
)

# Access typed custom field values
opp = Opportunity.model_validate(opp_data)
print(opp.custom_fields.legacy_id.value)   # typed as int
print(opp.custom_fields.group_name.value)   # typed as str
```
...
// Extract typed values from custom fields
...
if (agency) {
  ...
  console.log(agency.name); // typed as string | undefined
}
```
...
# Access custom fields directly as typed attributes
print(opp.custom_fields.legacy_id.value)    # typed as int
print(opp.custom_fields.group_name.value)   # typed as str
```
