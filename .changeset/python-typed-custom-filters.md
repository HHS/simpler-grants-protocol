...
- `opportunities.search()` raises `FilterError` on an invalid value for a standard or registered filter (or a conflicting `status` shorthand) before sending the request; ad-hoc (unregistered) keys stay best-effort and are dropped if unusable. `filter_info.errors` now carries server-returned errors only.
