import { NewComplianceError, ErrorType } from "./types";
...
export class ErrorCollection {
  ...
  addErrors(errors: NewComplianceError[]): void {
  ...
  filterByEndpoint(endpoint: string): ErrorCollection {
    const filtered = new ErrorCollection();
    filtered.addErrors(this.errors.filter(error => error.endpoint === endpoint));
    return filtered;
  }
  ...
  getAllErrors(): NewComplianceError[] {
    return [...this.errors];
  }
   ...
   * ```
   */
  [Symbol.iterator](): Iterator<NewComplianceError> {
