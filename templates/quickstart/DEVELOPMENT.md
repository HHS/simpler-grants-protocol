...
| `pnpm run format`       | Run automatic formatting and fix issues                          |
| `pnpm run check:format` | Check formatting, fail if issues are found                       |
| `pnpm run checks`       | Run all CI checks listed above                                   |
...
- [ ] A new project has been initialized from the feature branch version of this template using `tsp init <path-to-raw-template.json> --template quickstart` followed by `pnpm install` (note: `tsp init` does not support pnpm auto-install, so dependencies must be installed manually)
