...
import { errorResponse, successResponse, type FieldError } from "../http/envelope";
...
function applyStatusFilter(items: Opportunity[], filter: StringArrayFilter): Opportunity[] {
 ...
 */
function orderBy(items: Opportunity[], sortBy: string, sortOrder: string): Opportunity[] {
  return [...items].sort((a, b) => {
    const result = compare(sortKey(a, sortBy), sortKey(b, sortBy)) || compare(a.id, b.id);
...
function resolvePagination(
  ...
  const page = resolvePaginationParam(rawPage, `${fieldPrefix}page`, DEFAULT_PAGE, errors);
  ...
  return errors.length > 0 ? { ok: false, errors } : { ok: true, page, pageSize };
}
 ...
 */
export function listOpportunities(request: Request, version: Version): Response {
  ...
  if (!pagination.ok) {
    return errorResponse(400, "Invalid pagination parameters", pagination.errors);
  }
 ...
 */
export async function searchOpportunities(request: Request, version: Version): Promise<Response> {
  let parsed: unknown;
  try {
  ...
  if (sorting) {
    ...
    if (sorting.sortOrder !== undefined && !VALID_SORT_ORDER.has(sorting.sortOrder)) {
      errors.push({
  ...
  if (body.search) {
    ...
    items = items.filter(
      opp => opp.title.toLowerCase().includes(q) || opp.description.toLowerCase().includes(q)
    );
  }
  if (filters.status) {
  ...
  return successResponse({
    ...
    sortInfo: {
      sortBy,
      ...(sorting?.customSortBy !== undefined ? { customSortBy: sorting.customSortBy } : {}),
      sortOrder,
