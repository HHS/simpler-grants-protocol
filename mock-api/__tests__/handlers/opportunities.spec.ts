...
describe("opportunities routes", () => {
  describe("GET /v{version}/common-grants/opportunities (list)", () => {
    it("returns a 200 with all fixtures sorted by lastModifiedAt descending and default pagination", async () => {
      ...
      for (let i = 1; i < body.items.length; i++) {
        expect(new Date(body.items[i - 1].lastModifiedAt).getTime()).toBeGreaterThanOrEqual(
    ...
    it("returns an empty items array with valid paginationInfo when page is past the end", async () => {
      const response = await worker.fetch(new Request(opportunitiesUrl("0.3.0", "?page=999")));
    ...
    it("honors a pageSize above the default, since the spec declares no maximum", async () => {
      const response = await worker.fetch(new Request(opportunitiesUrl("0.3.0", "?pageSize=9999")));
    ...
    it.each([
      ...
      async (param, value) => {
        ...
        expect(typeof body.message).toBe("string");
        expect(body.errors).toEqual([{ field: param, message: "Must be at least 1" }]);
      }
    );

    it("returns 400 when a pagination param is present but not an integer", async () => {
      const response = await worker.fetch(new Request(opportunitiesUrl("0.3.0", "?pageSize=abc")));
  ...
  describe("GET /v{version}/common-grants/opportunities/{oppId} (detail)", () => {
    ...
    it.each(VERSIONS)("answers 200 for the id Swagger UI pre-fills, for v%s", async version => {
      const response = await worker.fetch(
    ...
    it("echoes the requested oppId and matches the list item's shared fields, while adding the detail-only competitions field", async () => {
      const listResponse = await worker.fetch(new Request(opportunitiesUrl("0.3.0")));
    ...
    it.each(VERSIONS)(
      ...
      async version => {
        const response = await worker.fetch(
          new Request(opportunitiesUrl(version, `/${RESERVED_MISSING_OPPORTUNITY_ID}`))
        );
    ...
    it.each(VERSIONS)(
      ...
      async version => {
        const response = await worker.fetch(new Request(opportunitiesUrl(version, "/not-a-uuid")));
    ...
    it("omits competitions and acceptedApplicantTypes from a v0.1 detail response", async () => {
      const response = await worker.fetch(new Request(opportunitiesUrl("0.1.0", `/${STEM_ID}`)));
  ...
  describe("determinism", () => {
    it.each(VERSIONS)(
      ...
      async version => {
        const responseA = await worker.fetch(new Request(opportunitiesUrl(version)));
        const responseB = await worker.fetch(new Request(opportunitiesUrl(version)));
    ...
    it.each(VERSIONS)(
      ...
      async version => {
        ...
        const responseA = await worker.fetch(new Request(opportunitiesUrl(version, `/${STEM_ID}`)));
        const responseB = await worker.fetch(new Request(opportunitiesUrl(version, `/${STEM_ID}`)));
    ...
    it.each(VERSIONS)(
      ...
      async version => {
        ...
        const listResponse = await worker.fetch(new Request(opportunitiesUrl(version)));
  ...
  describe("POST /v{version}/common-grants/opportunities/search", () => {
    ...
    it("answers GET on the search path with the detail handler's 400, as the spike's :oppId pattern did", async () => {
      const response = await worker.fetch(new Request(opportunitiesUrl("0.3.0", "/search")));
    ...
    it("filters using only the given bound when a maxAwardAmountRange filter omits max", async () => {
      ...
      for (const item of body.items) {
        ...
        expect(Number(fullItem!.funding!.maxAwardAmount!.amount)).toBeGreaterThanOrEqual(50000);
      }
    });
    ...
    it.each([
      ...
      async (field, filter, expectedErrorField) => {
        ...
        expect(body.status).toBe(400);
        expect(body.errors.map(error => error.field)).toContain(expectedErrorField);
      }
    );
...
describe("version path prefix", () => {
  it.each(VERSIONS)("resolves the list, detail, and search routes for v%s", async version => {
    const listResponse = await worker.fetch(new Request(opportunitiesUrl(version)));
  ...
  it("returns 404 with the protocol error envelope when the version prefix is missing entirely", async () => {
    const response = await worker.fetch(
      new Request("https://mock.example/common-grants/opportunities")
    );
  ...
  it("returns 404 with the protocol error envelope for a malformed version prefix", async () => {
    const response = await worker.fetch(
      new Request("https://mock.example/vabc/common-grants/opportunities")
    );
