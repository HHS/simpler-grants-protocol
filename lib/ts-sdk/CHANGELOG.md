...
- 5a9d09f: Typed custom-filters authoring surface and plugin-scoped client for the TypeScript SDK:
  ...
  - Breaking: `new Client(...)` no longer accepts `routes` (bind them via `plugin.getClient()`); `classifyFilters` now throws `FilterError` instead of returning collected errors, and `ClassifyResult` is removed.
  - Breaking: the `Opportunities` constructor is now `(client, boundSchema?, routes?)` — `routes` moved from the second to the third parameter (a compile error at existing two-argument call sites).
  - Breaking: the `schema` option on `Client.fetchMany` requires a `.safeParse()` method (any Zod schema qualifies); bare `{ parse }` objects are no longer accepted.
