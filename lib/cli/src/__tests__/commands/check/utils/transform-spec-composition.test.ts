import { transformSpecCompositionToCg, fixSchemaComposition, detectCompositionIssues } from '../../../../commands/check/utils/transform-spec-composition';
...
describe('transform-spec-composition', () => {
  describe('fixSchemaComposition', () => {
    it('should remove type field when type is [object] and allOf exists', () => {
      ...
      expect('allOf' in (result as any)).toBe(false);
    });
  ...
  describe('transformSpecCompositionToCg', () => {
    it('should transform response schemas with $ref', () => {
      ...
      const responseSchema = response && 'content' in response 
      ...
      expect('type' in (responseSchema as any)).toBe(false);
      expect('allOf' in (responseSchema as any)).toBe(false);
    });
    ...
    it('should transform component schemas', () => {
      ...
      expect('type' in (transformedSchema as any)).toBe(false);
      expect('allOf' in (transformedSchema as any)).toBe(false);
    });
  });
