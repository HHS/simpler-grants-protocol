...
describe("Logging utils", () => {
  describe("TargetType enum", () => {
    it("should have correct values for all target types", () => {
      expect(TargetType.Model).toBe("Schema");
      expect(TargetType.ModelProperty).toBe("field");
      expect(TargetType.Enum).toBe("Schema");
  ...
  describe("Log.added", () => {
    it("should generate correct messages for Schema types (Model and Enum)", () => {
      expect(Log.added(TargetType.Model, "User")).toBe("Schema created");
      expect(Log.added(TargetType.Enum, "Status")).toBe("Schema created");
    });

    it("should generate correct messages for other target types", () => {
    ...
    it("should handle parent name parameter (even though not used in current implementation)", () => {
      expect(Log.added(TargetType.ModelProperty, "name")).toBe(
        "Added `name` field"
      );
      expect(Log.added(TargetType.EnumMember, "active")).toBe(
  ...
  describe("Log.removed", () => {
    it("should generate correct messages for Schema types (Model and Enum)", () => {
      expect(Log.removed(TargetType.Model, "User")).toBe("Removed schema");
      expect(Log.removed(TargetType.Enum, "Status")).toBe("Removed schema");
    });
  ...
  describe("Log.renamedFrom", () => {
    it("should generate correct messages for all target types", () => {
      expect(Log.renamedFrom(TargetType.Model, "OldUser", "NewUser")).toBe(
        "Renamed Schema from `OldUser` to `NewUser`"
      );
      expect(Log.renamedFrom(TargetType.Enum, "OldStatus", "NewStatus")).toBe(
        "Renamed Schema from `OldStatus` to `NewStatus`"
      );
      expect(
